...
# read in HOLC data
holc <- st_make_valid(st_read(here("data", "mapping-inequality", "mapping-inequality-los-angeles.json")))
```
...
# select columns of interest to make wrangling use less memory
  select(ID, STATE_NAME, ST_ABBREV, CNTY_NAME, REGION, ACSTOTPOP, LOWINCPCT, P_PM25, P_LIFEEXPPCT) %>% 
...
```{r}
# join spatially 
# some ej_screen data have 2 or more HOLC grades associated to them, so we join using st_within
